...
TEST(DijkstraTest, BasicShortestPaths) 
{
    ...
    EXPECT_DOUBLE_EQ(result.at(1), 3.0);  // 0->2->1
    EXPECT_DOUBLE_EQ(result.at(2), 2.0);  // прямое соединение
    EXPECT_DOUBLE_EQ(result.at(3), 6.0);  // 0->2->1->3
}
...
TEST(DijkstraTest, PathUpdate) 
{
    ...
    auto result = graph.dijkstra(0);
    EXPECT_DOUBLE_EQ(result.at(1), 5.0);
    
    graph.removeVertex(2, 1);
    graph.addVertex(1, 1.0, 2);
    result = graph.dijkstra(0);
    EXPECT_DOUBLE_EQ(result.at(1), 5.0); // Путь остается 0->1
}
...
TEST(DijkstraTest, ComplexGraph) 
{
    ...
    EXPECT_DOUBLE_EQ(result.at(4), 20.0); // 0->2->5->4 (9+2+9)
    EXPECT_DOUBLE_EQ(result.at(3), 20.0); // 0->2->3 (9+11)
}
